...
namespace EdgeDetect_IP {
  ...
  public:
                        ...
                        )
    {
      ...
      int10 pix_result_dy, pix_result_dx;
      int11 max_val, pix_result;
        ...
        VCOL: for (maxWType x = 0; ; x++) {
          ...
          */
          ...
          if(ctrl_signal == 0){
            ...
            // Laplacian Filter
            pix_result = (pix[6]*Laplacian_kernel[0]) + (pix[3]*Laplacian_kernel[1]) + (pix[0]*Laplacian_kernel[2]) + 
                         (pix[7]*Laplacian_kernel[3]) + (pix[4]*Laplacian_kernel[4]) + (pix[1]*Laplacian_kernel[5]) + 
                         (pix[8]*Laplacian_kernel[6]) + (pix[5]*Laplacian_kernel[7]) + (pix[2]*Laplacian_kernel[8]) ;
          }else if(ctrl_signal == 2){       
            // Sobel Filter
            pix_result_dx = (pix[6]*Sobeldx_kernel[0]) + (pix[3]*Sobeldx_kernel[1]) + (pix[0]*Sobeldx_kernel[2]) + 
                            (pix[7]*Sobeldx_kernel[3]) + (pix[4]*Sobeldx_kernel[4]) + (pix[1]*Sobeldx_kernel[5]) + 
                            (pix[8]*Sobeldx_kernel[6]) + (pix[5]*Sobeldx_kernel[7]) + (pix[2]*Sobeldx_kernel[8]) ; 
            //printf(" ======== dx_buf=%2d ========= ",dx_buf);
            pix_result_dy = (pix[6]*Sobeldy_kernel[0]) + (pix[3]*Sobeldy_kernel[1]) + (pix[0]*Sobeldy_kernel[2]) + 
                            (pix[7]*Sobeldy_kernel[3]) + (pix[4]*Sobeldy_kernel[4]) + (pix[1]*Sobeldy_kernel[5]) + 
                            (pix[8]*Sobeldy_kernel[6]) + (pix[5]*Sobeldy_kernel[7]) + (pix[2]*Sobeldy_kernel[8]) ; 
